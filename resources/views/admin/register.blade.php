...
@section('content')
    ...
    @endif
    <form method="POST" action="{{ route('admin.register.submit', ['token' => $invitation->token]) }}"
        class="reservation-form menu-form" id="admin-register-form">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="password" id="password" name="password" required @error('password') class="is-invalid" @enderror>
            @error('password')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
