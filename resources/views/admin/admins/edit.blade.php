...
@endif
<form method="POST" action="{{ route('admin.admins.update', $admin) }}" enctype="multipart/form-data" class="reservation-form menu-form">
    @csrf
    @method('PUT')
    <div class="form-group">
        <label for="name">{{ __('messages.name') }}</label>
        <input type="text" id="name" name="name" value="{{ old('name', $admin->name) }}" @error('name') class="is-invalid" @enderror>
        @error('name')
    ...
    <div class="form-group">
        ...
        <div style="margin-top:5px;">
            <img id="avatarPreview" src="{{ $admin->profile_image_url }}" 
                 alt="Profile" 
