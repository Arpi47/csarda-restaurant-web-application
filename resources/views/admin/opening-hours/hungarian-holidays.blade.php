...
@section('title', __('messages.hungarian_holidays'))
@section('content') <h1>{{ __('messages.hungarian_holidays') }}</h1>
    <div class="contact-section">
        <h2>{{ __('messages.hungarian_holidays_management') }}</h2>
            ...
            <table class="opening-hours-table">
                ...
                <tbody>
                    @forelse ($holidays as $holiday)
                        <tr>
                            ...
                            <td>
                                <label class="opening-hours-checkbox">
                                    <input type="hidden" name="restaurant_is_active" value="0"
                                        form="hungarian-holiday-form-{{ $holiday->id }}">
                            ...
                            <td class="opening-hours-actions">
                                <button type="submit" form="hungarian-holiday-form-{{ $holiday->id }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="9">
                                {{ __('messages.no_hungarian_holidays') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @foreach ($holidays as $holiday)
            <form id="hungarian-holiday-form-{{ $holiday->id }}" method="POST"
                action="{{ route('admin.opening-hours.hungarian-holidays.update', $holiday) }}"
                class="admin-hungarian-holiday-form">
                @csrf
                @method('PUT')
            </form>
        @endforeach
