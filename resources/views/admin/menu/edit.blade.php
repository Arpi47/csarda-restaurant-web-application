...
@section('content')
<h1>{{ __('messages.edit_menu_item') }}</h1>
<form id="menu-form" method="POST" action="{{ route('admin.menu.update', $menu) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    @php
        $languages = [
        ...
        <fieldset>
            ...
            <div class="form-group">
                ...
                <textarea name="description_{{ $langKey }}" 
                          ...
                          required>{{ old('description_'.$langKey, $menu->{'description_'.$langKey}) }}</textarea>
    ...
    <div class="form-group">
        ...
        @error('image')
            <span class="text-danger">⚠ {{ $message }}</span>
        @enderror
        <small>{{ __('messages.leave_blank_if_no_change') }}</small>
    </div>
